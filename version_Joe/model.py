...
class DGMNet(tf.keras.Model):
    def __init__(self, config):
        super(DGMNet, self).__init__(name = config["nn_name"] + ".init_layer")
        ...
        self.g1_layer = DenseLayer(layer_width, 1, transformation="tanh")
        self.g2_layer = DenseLayer(layer_width, 1, transformation="tanh")
        self.g3_layer = DenseLayer(layer_width, 1, transformation="tanh")
        self.final_layer = DenseLayer(1, layer_width, transformation="tanh")

    def call(self, x):
        S = self.initial_layer.call(x)
        for i in range(self.n_layers):
            S = self.LSTMLayerList[i].call(S, x)
        R, log_R, log_1_minus_R = x[:, 1], tf.math.log(x[:, 1]), tf.math.log(1 - x[:, 1])
        g1 = self.g1_layer(tf.expand_dims(R, axis=-1))
        g2 = self.g2_layer(tf.expand_dims(log_R, axis=-1))
        g3 = self.g3_layer(tf.expand_dims(log_1_minus_R, axis=-1))
        combined = S * g1 + g2 + g3
        result = self.final_layer(combined)
        return result
...
class model:
    ...
    def __init__(self, params):
        ...
        '''    
        self.v_nn    = DGMNet(self.params['v_nn_config_5'])
        self.i_g_nn  = DGMNet(self.params['i_g_nn_config_5'])
        self.i_d_nn  = DGMNet(self.params['i_d_nn_config_5'])  
        ...
        if "pre_" in self.params["model_type"] and "post_" in self.params["model_type"]:
            ...
            self.v_post_tech_post_damage_nn = DGMNet(self.params['v_nn_config_5'])    
        ...
        if "pre_tech" in self.params["model_type"] and "pre_damage" in self.params["model_type"]:
            ...
            self.v_pre_tech_post_damage_nn = DGMNet(self.params['v_nn_config_6'])
            ...
            self.v_post_tech_pre_damage_nn = DGMNet(self.params['v_nn_config_4'])
            ...
            self.i_I_nn  = DGMNet(self.params['i_I_nn_config_5'])

        if "pre_tech" in self.params["model_type"] and "post_damage" in self.params["model_type"]:
            print("Pre tech model detected. Building a neural network for i_I")
            #self.i_I_nn  = FeedForwardSubNet(self.params['i_I_nn_config'])
            self.i_I_nn  = DGMNet(self.params['i_I_nn_config_6'])
            self.v_nn    = DGMNet(self.params['v_nn_config_6'])
            self.i_g_nn  = DGMNet(self.params['i_g_nn_config_6'])
            self.i_d_nn  = DGMNet(self.params['i_d_nn_config_6'])  

        if "post_tech" in self.params["model_type"] and "pre_damage" in self.params["model_type"]:
            print("Pre tech model detected. Building a neural network for i_I")
            self.v_nn    = DGMNet(self.params['v_nn_config_4'])
            self.i_g_nn  = DGMNet(self.params['i_g_nn_config_4'])
            self.i_d_nn  = DGMNet(self.params['i_d_nn_config_4']) 
